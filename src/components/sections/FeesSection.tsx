    ...
    <section id="fees" className="bg-sage py-24 md:py-32">
      <div className="max-w-5xl mx-auto px-6 md:px-12">
        <div className="text-center mb-16">
          <span className="text-emerald font-sans text-[10px] tracking-[0.25em] uppercase font-medium">Pricing</span>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan, idx) => (
            <div
              key={plan.name}
              className={`bg-background border rounded-2xl p-8 transition-all duration-500 hover:shadow-md ${
                idx === 0 ? "border-emerald/30 ring-1 ring-emerald/10" : "border-eucalyptus/60"
              }`}
            >
              {idx === 0 && (
                <span className="inline-block px-3 py-1 bg-emerald-bright text-white text-[10px] font-sans font-medium rounded-full mb-4 tracking-wider uppercase">
                  Most Popular
                </span>
              )}
                  ...
                  <div key={feature.text} className="flex items-start gap-2.5">
                    ...
                    <span
                      className={`font-sans text-sm ${
                        feature.included ? "text-charcoal/80" : "text-muted-foreground line-through"
                      }`}
                    >
                      {feature.text}
                    </span>
              ...
              <a
                ...
                className={`block text-center py-3 rounded-lg font-sans font-medium text-sm transition-all ${
                  idx === 0
                    ? "bg-emerald-bright text-white hover:opacity-90"
                    : "border border-eucalyptus/60 text-charcoal/70 hover:border-emerald/30 hover:text-emerald"
                }`}
              >
                Enroll Now
              </a>
